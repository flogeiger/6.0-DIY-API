...
app.get("/jokes", (req, res) => {
  ...
  if (jokeType) {
    const filteredJokes = jokes.filter(j => j.jokeType.toLowerCase() === jokeType.toLowerCase());
    res.json(filteredJokes);
  } else {
    res.status(400).json({ error: "Joke type is required" });
  }
});
//4. POST a new joke
app.post("/jokes", (req, res) => {
  ...
  jokes.push(newJoke);
  res.status(201).json(newJoke); // Return the new joke object
});
...
app.put("/jokes/:id", (req, res) => {
  ...
  if (jokeIndex !== -1) {
    jokes[jokeIndex] = { id: jokeId, jokeText: req.body.jokeText, jokeType: req.body.jokeType };
